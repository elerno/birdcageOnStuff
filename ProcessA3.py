...
### Amplitude methods.
	...
	def mkdistsToGaussPeak(self):
		...
		while len(dists) != self.harmNo:
			dist = random.gauss(self.dur * 0.5, .1)
			if dist < self.dur and dist > 0.03:
				dists.append(dist)
		return dists
	...
	def mkdistsToBetaPeak(self):
		...
		while len(dists) != self.harmNo:
			...
			dist = Scaling.valToRng(dist, 0, 1, .020, self.durToNxt)
			dists.append(dist)
		return dists
...
#			self.walkStrt	= tuple(self.newPitch)
	...
	def mkNote(self):
		...
		for freqEnv, amp, dist in zip(self.freqEnvlp, self.amps, self.diststoPeak):
			...
			self.NoteInstr1.dur				= self.dur
			self.NoteInstr1.ampPts			= [-140, (self.dur * .5, amp),
												(self.dur * .5, -140)]
	...
	def updateStrtDur2(self):
		...
		if self.countDown <= 50:
			if not len(self.distSer):
				if self.instrt % 5 == 0:
					self.distSer = Series.even(49)
					if self.growShrink == -1:
						self.distSer.reverse()

				if self.instrt % 5 == 1:
					self.distSer = range(1, 100)
					if self.growShrink == -1:
						self.distSer.reverse()

				if self.instrt % 5 == 2:
					self.distSer = Series.fibo(11)
					if self.growShrink == -1:
						self.distSer.reverse()

				if self.instrt % 5 == 3:
					self.distSer = Series.odd(50)
					if self.growShrink == -1:
						self.distSer.reverse()

				if self.instrt % 5 == 4:
					self.distSer = Series.prime(25)
					if self.growShrink == -1:
						self.distSer.reverse()

			self.distrs = random.choice(self.distSer)
			self.spctrm1	= self.spctrm2
			self.spctrm2	= Spctrm.dSpect(self.fundFreq1, self.prtls,
											self.distrs)
