...
class Process1(object):
	...
	def __init__(self, initCll=(56, 24),
		...
		# Start-times.
		self.offset		= offset
		self.rythmAtms	= [2, 3, 5, 7, 11, 13]
		self.StrtTms	= NoteMethods.StrtTms(self.rythmAtms, 0.1)
		self.strtsLin	= self.mkStrtsLin()
		self.logPulse	= Series.logar(60, len(self.iters) + 1, reverse=1)
		self.strts		= self.mkStrtsLog()
		# Durations
...
#### Start-time methods.
	...
	def mkStrtsLog(self):
		...
		for x in self.logPulse:
			strtsLog.append(x + self.offset)
		return strtsLog
...
#			iters.append(x)
		...
		for oneIter, strt, dur, ampPeaks, in zip(self.iters, self.strts, self.durs, self.ampPeaks):
			...
			if counter % modulo == 0:
				for cll, ampPeak in zip(oneIter, ampPeaks):
					self.NoteInstr1.vox			= self.coordVoxDict[str(cll)]
					self.NoteInstr1.strt		= strt
					self.NoteInstr1.dur		= (dura + random.uniform(0,
																		.02)) * -1

					self.NoteInstr1.ampPts	= [-960,
												(dura * .005, ampPeak - 9),
												(dura * .2, ampPeak - 15),
												(dura * .695, ampPeak - 15),
												(dura * .1, -960)]
